...
# sub sampling of our CNN architecture, for size of the initalization targets
nn_subsampling = 8
...
for scene in scenes:
    ...
    print("Reconstruction contains %d cameras and %d 3D points." %
          (num_cams, num_pts))
    ...
    for mode in modes:
        ...
        for cam_idx in range(num_cams):
            ...
            if image_file not in image_list:
                print("Skipping image " + image_file +
                      ". Not part of set: " + mode + ".")
                continue
            ...
            for pt_idx in range(0, pts_3D.size(0)):
                ...
                # projection to image
                img_pt = cam_pt[0:2, 0] * focal_length / \
                    cam_pt[2, 0] * out_scale
            ...
            torch.save(out_tensor, target_output_folder +
                       image_file[:-4] + '.dat')
